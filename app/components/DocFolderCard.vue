...
<template>
  <NuxtLink :to="`/doc/${folder.id}`" class="group flex max-w-48 cursor-pointer flex-col gap-2">
    ...
    <div class="flex flex-col gap-1 px-1">
      <h3 class="truncate text-base font-semi-bold text-white">{{ folder.title }}</h3>
      <div class="flex items-center gap-3">
        <div class="flex items-center gap-1.5">
          <div :class="['size-2.5 rounded-full', getStatusColor(folder.status)]"></div>
          <span class="text-sm font-semi-bold text-white/60">{{ formatStatus(folder.status) }}</span>
        </div>
        <span class="whitespace-nowrap text-sm font-semi-bold text-white/40">{{ folder.itemCount }} Items</span>
      </div>
    </div>
  </NuxtLink>
</template>
