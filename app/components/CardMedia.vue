...
async function onDownload(e: MouseEvent) {
  ...
  const blob = await $fetch<Blob>(props.media.url, { responseType: 'blob' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url
  a.download = `${props.media.id}.${props.media.type === 'video' ? 'mp4' : 'jpg'}`
  a.click()
...
<template>
  <NuxtLink
    ...
    <div class="absolute bottom-0 left-0 right-0 translate-y-full bg-gradient-to-t from-black/70 to-transparent px-1.5 pb-1.5 pt-4 transition-transform duration-300 group-hover:translate-y-0">
      <p class="truncate text-xs font-semi-bold leading-none text-white">{{ media.filename }}</p>
