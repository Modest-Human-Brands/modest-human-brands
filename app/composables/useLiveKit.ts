...
export function useLiveKit() {
  ...
  async function startPublishing(livekitUrl: string, token: string, videoEl: HTMLVideoElement, videoDeviceId?: string, audioDeviceId?: string) {
    ...
    try {
      ...
      await Promise.all([room.localParticipant.publishTrack(vTrack), room.localParticipant.publishTrack(aTrack)])
  ...
  async function startViewing(livekitUrl: string, token: string, videoEl: HTMLVideoElement) {
    ...
    try {
      const room = createRoom()
      registerViewerEvents(room, videoEl)
      await room.connect(livekitUrl, token)
      isConnected.value = true
      updateViewerState()
    } finally {
