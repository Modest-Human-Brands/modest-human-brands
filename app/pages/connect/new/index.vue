...
const { data: templates, pending } = await useFetch('/api/connect/email/template')
</script>

<template>
  <section class="relative flex h-full flex-col overflow-y-auto px-4 py-8 md:px-8">
    <header class="mb-8 flex flex-col gap-2">
      <h1 class="text-xl font-semi-bold tracking-tight text-white md:text-2xl">Email Templates</h1>
      <p class="text-sm text-light-500">Select a template to begin generating a new email.</p>
    </header>
    ...
    <div v-else-if="!templates?.length" class="flex min-h-[40vh] flex-col items-center justify-center text-center">
      ...
      <p class="mt-2 text-sm text-light-500">Create a email template in the system to get started.</p>
    </div>

    <div v-else class="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
      <TemplateCard v-for="template in templates" :id="template.id" :key="template.id" :label="template.label" :description="template.description" :link="`/connect/new/${template.id}`" />
    </div>
  </section>
</template>
