...
const { data: template } = await useFetch(`/api/connect/email/template/${templateId}`)
...
const triggerPreview = useDebounceFn(async () => {
  ...
  try {
    ...
    const res = await $fetch<{ contentHtml?: string }>('/api/connect/email/template/preview', {
      method: 'POST',
      body: {
    ...
    if (res?.contentHtml) {
      contentHtml.value = res.contentHtml
    }
  } catch (error: unknown) {
...
<template>
  <main class="relative flex size-full flex-row overflow-hidden bg-dark-400">
    ...
    <AppSidebar v-model:open="isDrawerOpen" as-drawer-on-mobile>
      ...
      <template #actions>
        <NuxtLink to="/connect/email/new" class="hidden size-9 items-center justify-center rounded-lg bg-dark-500 text-light-400 transition-colors hover:text-white md:flex">
