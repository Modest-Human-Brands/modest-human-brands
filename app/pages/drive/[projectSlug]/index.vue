...
<template>
  <div class="flex h-dvh flex-col overflow-hidden">
    ...
    <!-- Tabs -->
    <nav ref="tabs" class="scrollbar-none flex shrink-0 gap-1 overflow-x-auto border-b border-white/[0.06] py-2.5 sm:gap-1.5 sm:py-3">
      <button
        v-for="tab in tabs"
        :key="tab.id"
    ...
    <main ref="scroll" class="scrollbar-hidden flex-1 overflow-y-auto pb-16">
      ...
      <div v-else-if="filteredMedia && filteredMedia.length" class="grid grid-cols-2 gap-0.5 p-0.5 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
        <CardMediaPublic v-for="item in filteredMedia" :key="item.slug" :project-slug="slug" :media="item" />
      </div>
