import type { NotionProject } from '~~/shared/types'
  ...
  async run() {
    ...
    for await (const [slug, id] of Object.entries(assets)) {
      ...
      try {
        ...
        const originalWidth = 'stream' in metaData ? metaData.stream.width : metaData.format.width
        const originalHeight = 'stream' in metaData ? metaData.stream.height : metaData.format.height
        ...
        const [aW, aH] = aspectRatioLabel.split(':').flatMap((item) => Number.parseInt(item))
        const aspectRatio = aW / aH

        const { width: coverWidth, height: coverHeight } = calculateDimension(1080, aspectRatio)

        let updateCoverURL = `http://localhost:4345/media/image/s_${coverWidth}x${coverHeight}/${slug}`
        ...
        if (id) {
          ...
          const results = await notionQueryDb<NotionProject>(notion, notionDbId.project, {
            filter: {
              property: 'Index',
              number: { equals: Number.parseInt(slug.split('-')[1]) },
            },
            page_size: 10,
          })
          ...
          await notion.pages.create({
            ...
            properties: {
              ...
              Index: {
                type: 'number',
                number: Number.parseInt(slug.split('-')[2]),
              },
              'Version Index': {
                type: 'number',
                number: Number.parseInt(slug.split('-')[3]),
              },
              Name: {
                type: 'title',
                title: [
